...
public class SecurityUtil {
    ...
    private static boolean isLoggedIn() {
        ...
        return currentUser != null && currentUser.getPrincipal() != null && !StringUtils.isBlank(currentUser.getPrincipal().toString());
    }

    public static void assertIsNotLoggedIn() {
        if (isLoggedIn())
     ...
     *
     * @param dsm data stock meta
     * @throws IllegalArgumentException if invalid stock UUID
     * @throws AuthorizationException   if access is not to be granted
     */
     ...
     * @param dataSet data set to check for current user
     * @throws AuthorizationException if access is not to be granted
     */
     ...
     * exception is being thrown otherwise
     *
     * @param dsm data stock to check for current user
     * @throws AuthorizationException if access is not to be granted
     */
     ...
     * @throws AuthorizationException if access is not to be granted
     */
    public static void assertCanExport(IDataStockMetaData dsm) throws AuthorizationException {
        SecurityUtil.assertCan(dsm, "You are not permitted to export this data stock", ProtectionType.EXPORT);
    }
     ...
     * @param <T>     data set type
     * @param dataSet data set to check for current user
     * @param canWhat type of permissions to check
     * @throws AuthorizationException if access is not to be granted (generic message)
     */
     ...
     * @param <T>             data set type
     * @param dataSet         data set to check for current user
     * @param negativeMessage message of exception if permission is not to be granted
     * @param canWhat         type of permissions to check
     * @throws AuthorizationException if access is not to be granted (provided message)
     */
     ...
     *
     * @param <T>     data set type
     * @param dsm     data stock meta to check for current user
     * @param canWhat type of permissions to check
     * @throws AuthorizationException if access is not to be granted (generic message)
     */
    ...
    public static <T extends DataSet> void assertCan(IDataStockMetaData dsm, String negativeMessage, ProtectionType... canWhat) throws AuthorizationException {
        if (dsm == null) {
            log.trace("no datastock metadata specified, denying access");
            throw new AuthorizationException(negativeMessage);
        }
        log.trace("verifying permission for data stock with id {}", dsm.getId());
     ...
     *
     * @param u               user to check edit rights for
     * @param negativeMessage message for exception if user lacks edit permission
     * @throws AuthorizationException if permission is not present
     */
    public static void assertCanWrite(IUser u, String negativeMessage) throws AuthorizationException {
        if (u != null) {
     ...
     *
     * @param uid             user id to check edit rights for
     * @param negativeMessage message for exception if user lacks edit permission
     * @throws AuthorizationException if permission is not present
     */
     ...
     *
     * @param o organization to check for
     */
    public static void assertIsOrganizationAdmin(Organization o) {
        ...
        if (admGroup != null) {
            ...
            final IUser user = uab.getUserObject();
     ...
     * @param negativeMessage message for exception if user lacks permission
     * @throws AuthorizationException if permission is not present
     */
    public static void assertPermission(String permission, String negativeMessage) throws AuthorizationException {
        Subject sub = SecurityUtils.getSubject();
        if (isPermitted(permission)) {
            log.trace("permission {} granted", permission);
            return;
        }

        log.trace("permission {} DENIED", permission);
     ...
     */
    public static boolean hasReadPermission(DataSet ds) {
        return SecurityUtil.isPermitted(ds, ProtectionType.READ);
    }
     ...
     * Check a permission
     *
     * @param permission permission to check (wildcard string)
     * @return <code>true</code> if permission shall be granted, <code>false</code> otherwise
     */
    private static boolean isPermitted(String permission) {
        Subject sub = SecurityUtils.getSubject();

        log.trace("subject: {}, principal: {}", sub, sub.getPrincipal());

        if (sub.getPrincipal() == null) {
            // TODO: clean up later
            var guest_permissions = IlcdShiroPermissions.read(null); // ask db for guest permissions
            for (var p : guest_permissions)
                if (p.implies(new WildcardPermission(permission)))
                    return true;
        }

        if (sub != null) {
            log.trace("subject is permitted permission '{}': {}", permission, sub.isPermitted(permission));
     ...
     *
     * @param protectableType the protectable type to grant access to
     * @param protectionType  the protection type to grant access to as string
     * @param vals            identifier of objects to grant access to
     ...
     *
     * @param protectableType the protectable type to grant access to
     * @param protectionType  the protection types to grant access to
     * @param vals            identifier of objects to grant access to
     * @return Shiro wildcard string
     */
     ...
     *
     * @param protectableType the protectable type to grant access to
     * @param protectionType  the protection type to grant access to
     * @param vals            identifier of objects to grant access to
     * @return Shiro wildcard string
     */
    public static String toWildcardString(ProtectableType protectableType, ProtectionType protectionType, String vals) {
     ...
     * @param protectableType the protectable type to grant access to
     * @param protectionType  the protection type to grant access to
     * @param id              identifier of objects to grant access to
     * @return Shiro wildcard string
     */
    ...
    public static IUser getCurrentUser() {
        ...
        var principal = currentSubject.getPrincipal();

        if (principal instanceof FatPrincipal)
            return ((FatPrincipal) principal).getSodaUser();

        if (principal instanceof Pac4jPrincipal)
            return new UserOIDC(((Pac4jPrincipal) principal).getProfile());


//		if(principal == null) return new User(); 
        return new User(); // TODO: return null?
    }
