...
public enum ProtectionType {
    ...
    public static int combine(String... types) {
        // no values => return zero
        if (types == null || types.length == 0) {
            return 0;
        }
        ...
        for (var t : types) {
            if (t.equalsIgnoreCase("") || t.equalsIgnoreCase("*"))
                return (int) Math.pow(2, ProtectionType.values().length) - 1;
            result |= ProtectionType.valueOf(t.trim().toUpperCase()).getNumerical();
        }
        return result;
    }
     ...
     *
     * @param combinedNumerical a combined (via logical OR) value to check for current protection type
     ...
     *
     * @param apply      <code>true</code> to add current protection type, <code>false</code> to remove it
     * @param currentVal value to apply to
     * @return new value
     */
